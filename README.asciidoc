...
'install' them, 'update' them and 'remove' them, and more importantly it
...
version.  So 3.0 is a developer release while 3.1 is a stable release.
...
=== How to skip fetching the emacswiki recipes at install time?

The installer will fetch from emacswiki all the recipe available there and
install a local copy of those in your system automatically. Should you want
to disable that feature, the following snippet is showing how to:
