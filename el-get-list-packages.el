;;; el-get-list-packages.el --- Manage the external elisp bits and pieces you depend upon -*- lexical-binding: t; -*-
;;
    ...
    (let ((website (or website
      ...
      (when website
        (insert (el-get-fmt-button "Website: %s\n" website :type 'help-url
                                   'help-args (list website)))))
    (when descr
      (insert (format "Description: %s\n" descr)))
    (when depends
      (insert "Dependencies:")
      (cl-loop for d in (el-get-as-list depends)
               do (insert (el-get-fmt-button
                           " `%s'" (symbol-name d)
                           :type 'el-get-describe-package 'help-args (list d))))
      (insert ".\n"))
